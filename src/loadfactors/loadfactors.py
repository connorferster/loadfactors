...
def factored_max(load: Load, load_combinations: dict) -> Any:
    ...
    for load_combination, load_factors in load_combinations.items():
        current_fl = factor_load(np.array(load).T, np.array(load_factors))
...
def get_factored_matrix(load: Load, load_combinations: dict) -> np.ndarray:
    ...
    """
    acc = None
    for idx, (load_combination, load_factors) in enumerate(load_combinations.items()):
        current_fact = factor_load(np.array(load).T, np.array(load_factors))
        if idx == 0:
            acc = np.array([current_fact])
        else:
            acc = np.concatenate([acc, [current_fact]])
    return acc
